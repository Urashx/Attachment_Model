...
parents-own [energy social vision pahead children call otherparent]

breed [childs child]
childs-own [energy social security vision pahead caregivers with-cg cry summean sumvar]
...
to go
  let save_name (word "GroupCalling/results" date-and-time ".csv")
...
to winner_take_all
  ...
  if breed = childs [
    ...
    set list_motivation (list ( d_energy + incentive * d_energy * foodinvision) (d_social + incentive * d_social * childinvision) (d_security + autonomy))
...
to eat
 ...
 ifelse count foods-on pahead > 0
  [
    ...
    [
      let distance_child 999
      let me self
      ask my-links [
        set distance_child min list distance_child link-length
      ]

      ifelse distance_child >= 2 [
        set call 1
      ]
      [
...
to init-groups
  repeat number_of_groups [
    ...
    ask turtle-set cgs [
     set otherparent (other turtle-set cgs)
    ]
