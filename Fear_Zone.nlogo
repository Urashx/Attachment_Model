...
breed [childs child]
childs-own [energy social security vision pahead caregivers with-cg cry summean sumvar adaptticks]
...
to go
  let save_name (word "FearZone/results" date-and-time ".csv")
  ...
  if ticks = 36000 [
    ...
    file-open "dangermanagement.txt"
    file-print ( mean [ adaptticks ] of childs )
    file-close
    ;export-all-plots save_name
    setup
  ]
...
to winner_take_all
  ...
  if breed = childs [
    ...
    let lakeinvision 0
    if any? other lakes-on vision [
        set lakeinvision 1
        if with-cg = 1
        [
          set adaptticks ( adaptticks + 1 )
        ]
    ]
    let d_security (2 - security)
    set list_motivation (list ( d_energy + incentive * d_energy * foodinvision) (d_social + incentive * d_social * childinvision) (d_security + autonomy + incentive * d_security * lakeinvision))
...
to find-caregiver
  ...
  [
    set cry 1
    ifelse any? other lakes-on vision [
      face one-of other lakes-on vision
...
291
813
Mean number of ticks with behaviour adaptation
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [ adaptticks ] of childs"
